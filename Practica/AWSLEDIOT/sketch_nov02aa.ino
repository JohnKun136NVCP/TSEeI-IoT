...
String PUB_TOPIC = "embebed_cloud/Axolote/rgb";
String SUB_TOPIC = "embebed_cloud/Axolote/rgb";
...
void setup() {
  ...
  //*****************************
  // Cert leer archivo
  File file4 = SPIFFS.open("/097e-certificate.pem.crt", FILE_READ);
  if (!file4) {
    Serial.println("No se pudo abrir el archivo para leerlo");
    return;
  }
  ...
  File file6 = SPIFFS.open("/b8097e-private.pem.key", FILE_READ);
  if (!file6) {
    Serial.println("No se pudo abrir el archivo para leerlo");
    return;
  }
  ...
  report_status["state"]["reported"]["status"] = 1;
